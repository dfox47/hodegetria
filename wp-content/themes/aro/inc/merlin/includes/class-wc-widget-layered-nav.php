...
class Aro_Widget_Layered_Nav extends WC_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'woocommerce aro_widget_layered_nav woocommerce-widget-layered-nav';
        $this->widget_description = esc_html__('Display a list of attributes to filter products in your store.(Aro custom)', 'aro');
        $this->widget_id          = 'aro_woocommerce_layered_nav';
        $this->widget_name        = esc_html__('Aro Filter Products by Attribute', 'aro');
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            'title'      => array(
                'type'  => 'text',
                'std'   => esc_html__('Filter by', 'aro'),
                'label' => esc_html__('Title', 'aro'),
            ),
            'attribute'  => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => esc_html__('Attribute', 'aro'),
                'options' => $attribute_array,
            ),
            'query_type' => array(
                'type'    => 'select',
                'std'     => 'and',
                'label'   => esc_html__('Query type', 'aro'),
                'options' => array(
                    'and' => esc_html__('AND', 'aro'),
                    'or'  => esc_html__('OR', 'aro'),
                ),
            ),
        );
    }
    ...
    protected function layered_nav_list($terms, $taxonomy, $query_type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($count > 0 || $option_is_set) {
                ...
                $term_html = '<a rel="nofollow" class="aro-button-type" href="' . esc_url($link) . '">' . esc_html($term->name) . '</a>';

                if (class_exists('Woo_Variation_Swatches')) {
                    $attribute = Woo_Variation_Swatches_Backend::instance()->get_attribute_taxonomy($term->taxonomy);
                    $fields    = Woo_Variation_Swatches_Backend::instance()->attribute_meta_fields();
                    if ($attribute->attribute_type) {
                        $fields = isset($fields[$attribute->attribute_type]) ? $fields[$attribute->attribute_type] : array();
                    }
                    if (isset($fields[0]["type"])) {
                        ...
                        switch ($fields[0]["type"]) {
                            case 'color':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="aro-color-type" href="' . esc_url($link) . '">';
                            ...
                            case 'image':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="aro-image-type" href="' . esc_url($link) . '">';
