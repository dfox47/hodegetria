 ...
 * @extends WC_Widget
 */
class Aro_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'aro');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'aro');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'aro'),
				'label' => esc_html__('Title', 'aro'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'aro'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'aro'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'aro'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'aro'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'aro'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'aro'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'aro'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Aro_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'aro');
			...
			echo '<ul class="product-brands" id="aro-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Aro_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'aro'),
		'singular_name'              => esc_html__('Brands', 'aro'),
		'menu_name'                  => esc_html__('Brands', 'aro'),
		'all_items'                  => esc_html__('All Brands', 'aro'),
		'parent_item'                => esc_html__('Parent Brand', 'aro'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'aro'),
		'new_item_name'              => esc_html__('New Brand Name', 'aro'),
		'add_new_item'               => esc_html__('Add New Brands', 'aro'),
		'edit_item'                  => esc_html__('Edit Brand', 'aro'),
		'update_item'                => esc_html__('Update Brand', 'aro'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'aro'),
		'search_items'               => esc_html__('Search Brands', 'aro'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'aro'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'aro'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
