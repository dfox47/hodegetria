...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
    ...
    public function admin_page() {
        ...
        // Enqueue javascript.
        wp_enqueue_script('merlin', trailingslashit($this->base_url) . $this->directory . '/assets/js/merlin.js', array('jquery-core'), MERLIN_VERSION);

        $texts = array(
            'something_went_wrong' => esc_html__('Something went wrong. Please refresh the page and try again!', 'aro'),
        );
    ...
    public function svg($args = array()) {
        ...
        if (empty($args)) {
            return esc_html__('Please define default parameters in the form of an array.', 'aro');
        }

        // Define an icon.
        if (false === array_key_exists('icon', $args)) {
            return esc_html__('Please define an SVG icon filename.', 'aro');
        }
    ...
    public function steps() {
        ...
        if (!empty($this->import_files)) {
            $this->steps['content'] = array(
                'name' => esc_html__('Content', 'aro'),
                'view' => array($this, 'content'),
            );
        }

        $this->steps['ready'] = array(
            'name' => esc_html__('Ready', 'aro'),
    ...
    protected function welcome() {
        ...
        <?php
        $this->logger->debug(esc_html__('The welcome step has been displayed', 'aro'));
    }
    ...
    protected function license() {
        ...
        <?php
        $this->logger->debug(esc_html__('The license activation step has been displayed', 'aro'));
    }
    ...
    protected function child() {
        ...
        <?php
        $this->logger->debug(esc_html__('The child theme installation step has been displayed', 'aro'));
    }
    ...
    protected function plugins() {
                        ...
                        <?php foreach ($required_plugins as $slug => $plugin) : ?>
                            <li data-slug="<?php echo esc_attr($slug); ?>">
                                ...
                                <label for="default_plugins_<?php echo esc_attr($slug); ?>">
                                    ...
                                    <span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e('Required', 'aro'); ?>">
										<?php esc_html_e('req', 'aro'); ?>
									</span>
								</span>
                                </label>
                            </li>
        ...
        <?php
        $this->logger->debug(esc_html__('The plugin installation step has been displayed', 'aro'));
    }
    ...
    protected function content() {
                ...
                <div class="merlin__select-control-wrapper">
                    ...
                    <select class="merlin__select-control js-merlin-demo-import-select">
                        <?php foreach ($this->import_files as $index => $import_file) : ?>
                            <option value="<?php echo esc_attr($index); ?>" data-image="<?php echo esc_attr($import_file['import_preview_image_url']) ?>"><?php echo esc_html($import_file['import_file_name']); ?></option>
                        <?php endforeach; ?>
                    </select>

                    <div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__('Select Demo', 'aro'); ?>">
        ...
        <form action="" method="post" class="<?php echo esc_attr($multi_import); ?>">
            ...
            <ul class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
                <?php echo sprintf('%s', $this->get_import_steps_html($import_info)); ?>
            </ul>
        ...
        <?php
        $this->logger->debug(esc_html__('The content import step has been displayed', 'aro'));
    }
    ...
    protected function ready() {
        ...
        <?php
        $this->logger->debug(esc_html__('The final step has been displayed', 'aro'));
    }
    ...
    public function generate_child() {
        ...
        if (!file_exists($path)) {
            ...
            $this->logger->debug(esc_html__('The existing child theme was activated', 'aro'));
        ...
        $this->logger->debug(esc_html__('The newly generated child theme was activated', 'aro'));
    ...
    public function _ajax_activate_license() {
        ...
        if (!check_ajax_referer('merlin_nonce', 'wpnonce')) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__('Yikes! The theme activation failed. Please try again or contact support.', 'aro'),
                )
            );
        }
        ...
        if (empty($_POST['license_key'])) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__('Please add your license key before attempting to activate one.', 'aro'),
                )
            );
        }
        ...
        $this->logger->debug(esc_html__('The license activation was performed with the following results', 'aro'), $result);
    ...
    protected function edd_activate_license($license) {
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if (is_wp_error($response)) {
                ...
                $message = esc_html__('An error occurred, please try again.', 'aro');
            }
        } else {
                ...
                switch ($license_data->error) {
                    ...
                    case 'expired':
                        ...
                        /* translators: Expiration date */
                            esc_html__('Your license key expired on %s.', 'aro'),
                            date_i18n(get_option('date_format'), strtotime($license_data->expires, current_time('timestamp')))
                        );
                        break;

                    case 'revoked':
                        $message = esc_html__('Your license key has been disabled.', 'aro');
                        break;

                    case 'missing':
                        $message = esc_html__('This appears to be an invalid license key. Please try again or contact support.', 'aro');
                        break;

                    case 'invalid':
                    case 'site_inactive':
                        $message = esc_html__('Your license is not active for this URL.', 'aro');
                        break;

                    case 'item_name_mismatch':
                        /* translators: EDD Item Name */
                        $message = sprintf(esc_html__('This appears to be an invalid license key for %s.', 'aro'), $this->edd_item_name);
                        break;

                    case 'no_activations_left':
                        $message = esc_html__('Your license key has reached its activation limit.', 'aro');
                        break;
    ...
    public function generate_child_functions_php($slug) {
        ...
        $this->logger->debug(esc_html__('The child theme functions.php content was generated', 'aro'));
    ...
    public function generate_child_style_css($slug, $parent, $author, $version) {
        ...
        $this->logger->debug(esc_html__('The child theme style.css content was generated', 'aro'));
    ...
    public function generate_child_screenshot($path) {
        ...
        if (!empty($screenshot) && file_exists($screenshot)) {
            ...
            $this->logger->debug(esc_html__('The child theme screenshot was copied to the child theme, with the following result', 'aro'), array('copied' => $copied));
        } else {
            $this->logger->debug(esc_html__('The child theme screenshot was not generated, because of these results', 'aro'), array('screenshot' => $screenshot));
        }
    }
    ...
    function _ajax_plugins() {
        ...
        foreach ($plugins['activate'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Activating', 'aro'),
                );
                break;
            }
        }
        ...
        foreach ($plugins['update'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Updating', 'aro'),
                );
                break;
            }
        }
        ...
        foreach ($plugins['install'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Installing', 'aro'),
                );
                break;
            }
        }
        ...
        if ($json) {
            $this->logger->debug(
                esc_html__('A plugin with the following data will be processed', 'aro'),
                array(
            ...
            $json['message'] = esc_html__('Installing', 'aro');
            wp_send_json($json);
        } else {
            $this->logger->debug(
                esc_html__('A plugin with the following data was processed', 'aro'),
                array(
                    'plugin_slug' => $_POST['slug'],
                )
            );
            ...
            wp_send_json(
                array(
                    'done'    => 1,
                    'message' => esc_html__('Success', 'aro'),
                )
            );
        }
    ...
    function _ajax_content() {
        ...
        if (!check_ajax_referer('merlin_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])) {
            $this->logger->error(esc_html__('The content importer AJAX call failed to start, because of incorrect data', 'aro'));

            wp_send_json_error(
                array(
                    'error'   => 1,
                    'message' => esc_html__('Invalid content!', 'aro'),
                )
            );
        }
        ...
        if (isset($_POST['proceed'])) {
            if (is_callable($this_content['install_callback'])) {
                $this->logger->info(
                    esc_html__('The content import AJAX call will be executed with this import data', 'aro'),
                    array(
        ...
        if ($json) {
            ...
            $this->logger->error(
                esc_html__('The content import AJAX call failed with this passed data', 'aro'),
                array(
    ...
    public function _ajax_get_total_content_import_items() {
        if (!check_ajax_referer('merlin_nonce', 'wpnonce') && empty($_POST['selected_index'])) {
            $this->logger->error(esc_html__('The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'aro'));

            wp_send_json_error(
                array(
                    'error'   => 1,
                    'message' => esc_html__('Invalid data!', 'aro'),
                )
            );
        }
    ...
    public function get_import_data_info($selected_import_index = 0) {
        ...
        if (
            !empty($this->import_files[$selected_import_index]['homepage'])
        ) {
    ...
    protected function get_import_data($selected_import_index = 0) {
        ...
        if (!empty($import_files['content'])) {
            $content['content'] = array(
                'title'            => esc_html__('Content', 'aro'),
                'description'      => esc_html__('Demo content data.', 'aro'),
                'pending'          => esc_html__('Pending', 'aro'),
                'installing'       => esc_html__('Installing', 'aro'),
        ...
        if (!empty($import_files['widgets'])) {
            $content['widgets'] = array(
                'title'            => esc_html__('Widgets', 'aro'),
                'description'      => esc_html__('Sample widgets data.', 'aro'),
                'pending'          => esc_html__('Pending', 'aro'),
                'installing'       => esc_html__('Installing', 'aro'),
                'success'          => esc_html__('Success', 'aro'),
        ...
        if (!empty($import_files['sliders'])) {
            $content['sliders'] = array(
                'title'            => esc_html__('Revolution Slider', 'aro'),
                'description'      => esc_html__('Sample Revolution sliders data.', 'aro'),
                'pending'          => esc_html__('Pending', 'aro'),
                'installing'       => esc_html__('Installing', 'aro'),
                'success'          => esc_html__('Success', 'aro'),
        ...
        if (!empty($import_files['options'])) {
            $content['options'] = array(
                'title'            => esc_html__('Home Page', 'aro'),
                'description'      => esc_html__('Install homepage xml.', 'aro'),
                'pending'          => esc_html__('Pending', 'aro'),
                'installing'       => esc_html__('Installing', 'aro'),
                'success'          => esc_html__('Success', 'aro'),
                'install_callback' => array($this->importer, 'import'),
        ...
        if (!empty($import_files['redux'])) {
            $content['redux'] = array(
                'title'            => esc_html__('Redux Options', 'aro'),
                'description'      => esc_html__('Redux framework options.', 'aro'),
                'pending'          => esc_html__('Pending', 'aro'),
                'installing'       => esc_html__('Installing', 'aro'),
                'success'          => esc_html__('Success', 'aro'),
        ...
        if (false !== has_action('merlin_after_all_import')) {
            $content['after_import'] = array(
                'title'            => esc_html__('After import setup', 'aro'),
                'description'      => esc_html__('After import setup.', 'aro'),
                'pending'          => esc_html__('Pending', 'aro'),
                'installing'       => esc_html__('Installing', 'aro'),
                'success'          => esc_html__('Success', 'aro'),
     ...
     *
     * @param array $files Path to the revolution slider zip file.
     */
    public function import_revolution_sliders($files) {
...
//        $response = $importer->importSliderFromPost(true, true, $file);
        ...
        $this->logger->info(esc_html__('The revolution slider import was executed', 'aro'));
    ...
    public function pt_importer_new_ajax_request_response_data($data) {
        ...
        $data['message']  = esc_html__('Installing', 'aro');
        $data['proceed']  = 'true';
    ...
    public function after_content_import_setup() {
        ...
        if ($homepage) {
            ...
            $this->logger->debug(esc_html__('The home page was set', 'aro'), array('homepage_id' => $homepage));
        }
        ...
        if ($blogpage) {
            ...
            $this->logger->debug(esc_html__('The blog page was set', 'aro'), array('blog_page_id' => $blogpage));
        }
    }
    ...
    public function before_content_import_setup() {
        ...
        if (!empty($hello_world)) {
            ...
            $this->logger->debug(esc_html__('The Hello world post status was set to draft', 'aro'));
        }
    }
    ...
    public function validate_import_file_info($import_files) {
        ...
        foreach ($import_files as $import_file) {
            if (!empty($import_file['import_file_name'])) {
                ...
                $this->logger->warning(esc_html__('This predefined demo import does not have the name parameter: import_file_name', 'aro'), $import_file);
            }
        }
    ...
    public function get_import_files_paths($selected_import_index) {
        ...
        if (!empty($selected_import_data['homepage'])) {
            if (file_exists($selected_import_data['homepage'])) {
                $import_files['options'] = $selected_import_data['homepage'];
            }
        }
        ...
        if (!empty($selected_import_data['import_rev_slider_file_url'])) {
            ...
            // Retrieve the content import file.
            $file_path = $downloader->fetch_existing_file($rev_slider_filename);

            // Download the file, if it's missing.
            if (empty($file_path)) {
                $file_path = $downloader->download_file($selected_import_data['import_rev_slider_file_url'], $rev_slider_filename);
            }
            ...
            $import_files['sliders'][] = $file_path;
            if (!empty($selected_import_data['import_more_revslider_file_url'])) {
                // Setup filename paths to save the Redux content.
                foreach ($selected_import_data['import_more_revslider_file_url'] as $index => $slider_item) {
                    $slider_filename = 'slider-' . $index . '-' . $base_file_name . '.zip';

                    // Retrieve the content import file.
                    $file_path = $downloader->fetch_existing_file($slider_filename);

                    // Download the file, if it's missing.
                    if (empty($file_path)) {
                        $file_path = $downloader->download_file($slider_item, $slider_filename);
                    }
            ...
            if (!empty($selected_import_data['import_lookbook_revslider_file_url']) && !isset($check_oneclick['slidelookbook'])) {
                // Setup filename paths to save the Redux content.
                foreach ($selected_import_data['import_lookbook_revslider_file_url'] as $index => $slider_item) {
                    $slider_filename = 'slider-' . $index . '-' . $base_file_name . '.zip';
