<?php
/**
 * Aro WooCommerce Adjacent Products Class
 *
 * @package  aro
 * @since    2.4.3
 */
...
if ( ! class_exists( 'Aro_WooCommerce_Adjacent_Products' ) ) :

	/**
	 * The Aro WooCommerce Adjacent Products Class
	 */
	class Aro_WooCommerce_Adjacent_Products {

		private $_prefix = 'remove';
		...
		private function get_adjacent() {
			...
			$action( 'get_' . $direction . '_post_where', array( $this, 'filter_post_where' ) );
		...
		private function query_wc() {
			...
			$products = wc_get_products( apply_filters( 'aro_woocommerce_adjacent_query_args', $args ) );
