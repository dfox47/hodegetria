...
! defined( 'WOOSQ_VERSION' ) && define( 'WOOSQ_VERSION', '3.4.2' );
		...
		if ( ! class_exists( 'WPCleverWoosq' ) ) {
			class WPCleverWoosq {
					...
					if ( self::get_setting( 'add_to_cart_button', 'single' ) === 'archive' ) {
						...
						if ( $product->is_type( 'variation' ) ) {
							...
							<script type="text/javascript">
                              (function($) {
                                $('#woosq-popup input[name="add-to-cart"]').val(<?php echo esc_attr( $product_id ); ?>);
                                $('#woosq-popup input[name="product_id"]').val(<?php echo esc_attr( $product_id ); ?>);
                                $('#woosq-popup input[name="variation_id"]').
                                    val(<?php echo esc_attr( $variation_id ); ?>);
                              })(jQuery);
							</script>
							<?php
						} else {
				...
				function admin_enqueue_scripts( $hook ) {
					wp_enqueue_style( 'woosq-backend', WOOSQ_URI . 'assets/css/backend.css', [], WOOSQ_VERSION );
