		...
		if ( ! class_exists( 'WPCleverWoosc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoosc {
				...
				function __construct() {
					...
					// ajax search
					add_action( 'wp_ajax_woosc_search', [ $this, 'search' ] );
					add_action( 'wp_ajax_nopriv_woosc_search', [ $this, 'search' ] );

					// ajax load data
					add_action( 'wp_ajax_woosc_load_data', [ $this, 'load_data' ] );
					add_action( 'wp_ajax_nopriv_woosc_load_data', [ $this, 'load_data' ] );
					...
					// quick table
					if ( self::get_setting( 'quick_table_enable', 'yes' ) === 'yes' ) {
				...
				function admin_enqueue_scripts( $hook ) {
					wp_enqueue_style( 'woosc-backend', WOOSC_URI . 'assets/css/backend.css', [], WOOSC_VERSION );
					...
					<div class="wpclever_settings_page wrap">
						...
						<div class="wpclever_settings_page_content">
							<?php if ( $active_tab === 'settings' ) {
								...
								$click_outside        = self::get_setting( 'click_outside', 'yes' );
								$quick_table_enable   = self::get_setting( 'quick_table_enable', 'yes' );
								$quick_table_position = self::get_setting( 'quick_table_position', 'above_related' );
								...
								<form method="post" action="options.php">
									<table class="form-table">
										...
										<tr>
											...
											<td>
												<textarea name="woosc_settings[custom_attributes]" rows="10" cols="50" class="large-text"><?php echo self::get_setting( 'custom_attributes' ); ?></textarea>
										...
										<tr>
											...
											<td>
												<textarea name="woosc_settings[custom_fields]" rows="10" cols="50" class="large-text"><?php echo self::get_setting( 'custom_fields' ); ?></textarea>
											...
											<td>
												...
												if ( $nav_menus ) {
													...
													foreach ( $nav_menus as $nav_id => $nav_name ) {
														echo '<input type="checkbox" name="woosc_settings[menus][]" value="' . $nav_id . '" ' . ( is_array( $saved_menus ) && in_array( $nav_id, $saved_menus, false ) ? 'checked' : '' ) . '/><label>' . $nav_name . '</label><br/>';
													}
												} else {
				...
				function shortcode_list( $attrs ) {
					return '<div class="woosc_list woosc-list woosc_page woosc-page">' . self::get_table( false, null, 'page' ) . '</div>';
				}
